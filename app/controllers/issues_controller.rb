...
class IssuesController < ApplicationController
  ...
  def create
    ...
    if close_modal?
      unless authorized?(:edit_issue, @issue)
        session[:success] = t('issues.foreign_create_success', issue_id: @issue.id, group: @issue.group)
      end
  ...
  def update
    ...
    if @issue.update(issue_params) && close_modal?
      unless authorized?(:edit_issue, @issue)
        session[:success] = t('issues.foreign_update_success', issue_id: @issue.id, group: @issue.group)
      end
