...
module CitysdkResponder
  ...
  def citysdk_response(objects, options = {})
    ...
    respond_to do |format|
      ...
      format.any(:xml, :json) do
        render({ request.format.to_sym => objects, dasherize: false }.merge(options), status: options[:status] || :ok)
      end
    end
  end
